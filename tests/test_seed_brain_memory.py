...
def _seed_files(base):
    ...
    (base / "wiki" / "w1.md").write_text(
        "프론트매터 없는 위키 노트 본문.", encoding="utf-8"
    )
...
async def test_seed_memory_hardfail_on_non_ollama_embedder(tmp_path):
    ...
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_brain_memory_guard"
    )
...
async def test_seed_memory_allow_fallback_bypasses_guard(tmp_path):
    _seed_files(tmp_path)
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_brain_memory_bypass"
    )
...
async def test_seed_triples_hardfail_on_non_ollama_embedder(tmp_path):
    ...
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_ontology_guard"
    )
    with pytest.raises(HardFailEmbedderError):
...
async def test_seed_memory_idempotent_point_count(tmp_path):
    _seed_files(tmp_path)
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_brain_memory_idem"
    )
...
async def test_seed_triples_allow_fallback_and_idempotent(tmp_path):
    ...
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_ontology_bypass"
    )
...
async def test_seed_triples_missing_file_returns_zero(tmp_path):
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_ontology_missing"
    )
    stats = await seed_triples(
        qdrant=qa, triple_map_path=tmp_path / "nope.md", allow_fallback=True
    )
...
async def test_seed_memory_payload_shape(tmp_path):
    _seed_files(tmp_path)
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_brain_memory_payload"
    )
    await seed_memory(qdrant=qa, base_dir=tmp_path, allow_fallback=True)
    ...
    assert any(
        p["type"] == "brain:decisions" and p["title"] == "결정 하나" for p in payloads
    )
...
async def test_seed_memory_limit_option(tmp_path):
    _seed_files(tmp_path)
    (tmp_path / "wiki" / "w2.md").write_text(
        "두 번째 위키 노트 본문 내용.", encoding="utf-8"
    )
    qa = QdrantAdapter(
        url=None, embedder=HashEmbedder(), collection="test_brain_memory_limit"
    )
    stats = await seed_memory(
        qdrant=qa, base_dir=tmp_path, allow_fallback=True, limit=1
    )
...
async def test_seed_memory_incremental_skips_unchanged(tmp_path):
    ...
    s1 = await seed_memory(
        qdrant=qa, base_dir=base, allow_fallback=True, manifest_path=manifest
    )
...
async def test_seed_memory_incremental_reembeds_only_changed(tmp_path):
    ...
    (base / "wiki" / "w1.md").write_text(
        "수정된 위키 노트 본문(내용 변경).", encoding="utf-8"
    )
    s2 = await seed_memory(
        qdrant=qa, base_dir=base, allow_fallback=True, manifest_path=manifest
    )
    assert (
        s2["reembedded_files"] == 1
        and s2["unchanged_files"] == 1
        and s2["new_files"] == 0
    )
...
async def test_seed_memory_full_flag_reembeds_all(tmp_path):
    ...
    s2 = await seed_memory(
        qdrant=qa, base_dir=base, allow_fallback=True, manifest_path=manifest, full=True
    )
...
async def test_seed_memory_manifest_env_mismatch_treated_as_full(tmp_path):
    ...
    await seed_memory(
        qdrant=qa1, base_dir=base, allow_fallback=True, manifest_path=manifest
    )
    await qa1.aclose()

    qa2 = QdrantAdapter(
        url=None, embedder=CountingEmbedder(), collection="test_u8_mismatch_b"
    )
    s2 = await seed_memory(
        qdrant=qa2, base_dir=base, allow_fallback=True, manifest_path=manifest
    )
...
async def test_seed_memory_shrunk_file_reports_stale_chunks(tmp_path):
    ...
    long_md = "\n\n".join(
        "가" * 400 for _ in range(3)
    )  # CJK 1글자≈1토큰 → 512 한도로 다청크
    ...
    assert (
        rep["path"] == "wiki/w-long.md"
        and rep["new_chunks"] == 1
        and rep["old_chunks"] > 1
    )
    cnt2 = (await qa._get_client().count(qa.collection)).count
...
async def test_seed_memory_no_manifest_keeps_legacy_full_behavior(tmp_path):
    ...
    _seed_files(base)
    qa = QdrantAdapter(
        url=None, embedder=CountingEmbedder(), collection="test_u8_legacy"
    )
...
# ── 원문 역링크 푸터 제외 (상류 NaN 버그 유발 청크 2건이 전부 이거였다) ──
from scripts import seed_brain_memory as S  # noqa: E402


def test_blank_source_footers_removes_backlink():
    body = "본문 내용.\n\n**원문:** [열기](http://karpathy.github.io/2026/02/12/microgpt/)\n"
    out = S._blank_source_footers(body)
    assert "원문" not in out
    assert "본문 내용." in out


def test_blank_source_footers_preserves_line_numbers():
    """줄을 지우지 않고 비운다 — 지우면 뒤쪽 chunk_start_line 역링크가 전부 밀린다."""
    body = "첫 줄.\n\n**원문:** [열기](https://a.example/x/y)\n\n뒤 본문.\n"
    out = S._blank_source_footers(body)
    assert out.count("\n") == body.count("\n")
    assert out.split("\n").index("뒤 본문.") == body.split("\n").index("뒤 본문.")


def test_blank_source_footers_handles_multiple():
    """url.ts 는 본문 중간과 끝에 두 번 붙인다."""
    body = "가.\n\n**원문:** [열기](https://a.example/1)\n\n나.\n\n**원문:** [열기](https://a.example/2)\n"
    out = S._blank_source_footers(body)
    assert "열기" not in out
    assert "가." in out and "나." in out


def test_blank_source_footers_keeps_normal_links():
    """본문 안의 평범한 마크다운 링크는 건드리지 않는다."""
    body = "참고 [문서](https://a.example/doc) 를 보라.\n"
    assert S._blank_source_footers(body) == body


def test_blank_source_footers_noop_without_footer():
    body = "푸터 없는 일반 문서.\n\n두 번째 문단.\n"
    assert S._blank_source_footers(body) == body
